...
import { sampleHappVersion } from "../../../../ui/src/main/happs/mocks.js";
import { HappVersion } from "../../../../ui/src/main/happs/types.js";
...
test("create HappVersion", async () => {
  await runScenario(async scenario => {
    ...
    const happVersion: EntryRecord<HappVersion> = await alice.store.client.createHappVersion(
...
test("create and read HappVersion", async () => {
  await runScenario(async scenario => {
    ...
    // Alice creates a HappVersion
    const happVersion: EntryRecord<HappVersion> = await alice.store.client.createHappVersion(sample);
    assert.ok(happVersion);
...
test("create and update HappVersion", async () => {
  await runScenario(async scenario => {
    ...
    let updatedHappVersion: EntryRecord<HappVersion> = await alice.store.client.updateHappVersion(
      originalActionHash,
      originalActionHash,
      contentUpdate,
    );
