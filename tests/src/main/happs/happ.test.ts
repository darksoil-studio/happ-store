...
test('create and read Happ', async () => {
	await runScenario(async scenario => {
		...
		const createReadOutput: EntryRecord<Happ> = await toPromise(
			bob.store.happs.get(happ.actionHash).original,
		);
...
test('create and update Happ', async () => {
	await runScenario(async scenario => {
		...
		const readUpdatedOutput0: EntryRecord<Happ> = await toPromise(
			bob.store.happs.get(happ.actionHash).latestVersion,
		);
		assert.deepEqual(
			contentUpdate,
		...
		updatedHapp = await alice.store.client.updateHapp(
			originalActionHash,
			updatedHapp.actionHash,
			contentUpdate,
		);
		assert.ok(updatedHapp);
...
test('create and delete Happ', async () => {
	await runScenario(async scenario => {
		...
		const deleteActionHash = await alice.store.client.deleteHapp(
			happ.actionHash,
		);
		...
		const deletes: Array<SignedActionHashed<Delete>> = await toPromise(
			bob.store.happs.get(happ.actionHash).deletes,
		);
