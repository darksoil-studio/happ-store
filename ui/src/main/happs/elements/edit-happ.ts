...
export class EditHapp extends SignalWatcher(LitElement) {
  ...
  async firstUpdated() {
    const currentRecord = await toPromise(this.happsStore.happs.get(this.happHash).latestVersion);
    setTimeout(() => {
  ...
  renderEditForm(currentRecord: EntryRecord<Happ>) {
    return html`
      ...
      msg("Description")
        ...
        <upload-files name="icon" one-file accepted-files="image/jpeg,image/png,image/gif" required .defaultValue=${currentRecord.entry.icon}></upload-files>
