...
export class CreateHappVersion extends SignalWatcher(LitElement) {
  ...
  async createHappVersion(fields: Partial<HappVersion>) {
    if (this.happHash === undefined) throw new Error("Cannot create a new Happ Version without its happ_hash field");
    ...
    try {
      ...
      const record: EntryRecord<HappVersion> = await this.happsStore.client.createHappVersion(happVersion);
  ...
  render() {
    return html`
      <sl-card style="flex: 1;">
        <span slot="header">${msg("Create Happ Version")}</span>
        ...
        >  
          ...
          <sl-textarea name="changes" .label=${msg("Changes")}  required></sl-textarea>
          <upload-files name="web_happ_bundle_hash" one-file accepted-files="image/jpeg,image/png,image/gif" required></upload-files>
