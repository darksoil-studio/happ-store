  ...
  renderList(hashes: Array<ActionHash>) {
    if (hashes.length === 0) {
      return html` <div class="column center-content" style="gap: 16px;">
        ...
        ></sl-icon>
        <span class="placeholder">${msg("No happ versions found for this happ")}</span>
      </div>`;
    }

    return html`
      <div style="display: flex; flex-direction: column">
        ${hashes.map(hash => html`<happ-version-summary .happVersionHash=${hash}></happ-version-summary>`)}
      </div>
    `;
  }
